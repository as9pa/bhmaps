...
- With `UseWPF` on, the Windows Desktop implicit-usings set **omits `System.IO`** (and `System.Net.Http`). Every csproj carries an `<ItemGroup>` with `<Using Include="System.IO" />` (added in Task 1), so `Directory`, `File`, `Path`, and `IOException` resolve everywhere; never add a per-file `using System.IO;`.
...
- A global csharpier PostToolUse hook reformats .cs files on Write/Edit; the repo-root .csharpierignore (added in Task 5) turns it off here. Use the Write tool normally; do not route files through cp workarounds.
