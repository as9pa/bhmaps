...
`src\BhMaps.App\Views\HomeView.xaml` (line 398) and `src\BhMaps.App\Views\BackgroundsView.xaml`
(line 44), and `Padding="33,6,10,6"` to `Padding="33,12,10,12"` in `src\BhMaps.App\Controls\SuggestBox.xaml`
(line 11). The six other `FieldTextBox` sites, which today get 20 px of left inset instead of 10, self-correct.
...
### Task 19: the rename sweep, the manual note and the whole-part check
