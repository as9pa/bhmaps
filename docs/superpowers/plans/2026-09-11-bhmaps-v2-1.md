...
`Load` keeps its body, its view order and its `try`/`catch (OperationCanceledException)`; only the three collection types change, because the three tile classes are now one:

```csharp
    private async void Load(
        IReadOnlyList<PackTileViewModel> combined,
        IReadOnlyList<PackTileViewModel> backgrounds,
        IReadOnlyList<PackTileViewModel> platforms,
        Pack pack,
        CancellationToken ct)
```

Inside it, a tile whose `Map` is not null takes `await ComposeAsync(tile, pack, ct)`, and the middle loop, which used to lean on the tile's type, now guards on the file: `if (tile.File is { } file) { tile.Preview = await Shell.Services.Thumbnails.GetAsync(file.FullPath, file.MtimeTicks, ct); }`. `ComposeAsync` changes in its parameter type alone and reads `tile.Map!` behind the same `_snapshot?.Catalog.HasLevelData == true` check:
