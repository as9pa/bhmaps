...
`mapArt` tree, and copies them into the game folder on demand. It can also capture what the game has
right now as a `Default` pack, reset a map back to that, import loose images, and fit any picture to
the background size. The only files it changes are `.png` and `.jpg` files inside the `mapArt` folder
and inside its own library, plus its own files in `%APPDATA%`. The game's data files are read and
never written, and nothing else in the game install is touched.

The design notes are in `docs/superpowers/specs/2026-09-10-bhmaps-v2-design.md`; the v1 notes it
builds on are in `docs/superpowers/specs/2026-09-08-bhmaps-design.md`.
...
- Brawlhalla installed through Steam, or any folder shaped like `mapArt`: one level of subfolders
  holding `.png` and `.jpg` files. Real map names, level sets and composed previews also need the
  game's four data files in the folder above `mapArt`; without them the app still runs, with folder
  names and single-file thumbnails.
...
framework-dependent file of about 1.4 MB (the two `.pdb` files beside it are debug symbols and are
...
| Pack library | `Documents\BhMaps` by default; this machine uses `C:\Users\alexa\files\bh` |
...
write into the game folder was about to overwrite or delete.
...
- `System.IO` is a project-level global using in every `.csproj`; do not add `using System.IO;` to a
  file.
- `dotnet format BhMaps.slnx` is the only formatter this repo uses. csharpier is not installed here
  and must not be added. `.csharpierignore` at the repo root ignores every file, and exists only to
  switch off a csharpier formatting hook installed outside this repo.
